...
1. **JavaScript logs**. Including the main thread and the background thread.
   ...
   - By default, logs from the background thread are displayed in full format, while logs from the main thread are serialized as strings with the `lepus-console` prefix.
