...
The **Elements** panel allows you to inspect and modify DOM elements.

## Overview

The **Elements** panel provides a powerful interface for inspecting and manipulating the DOM. You can select specific DOM nodes using the DOM tree and use other tools to make modifications.
...
At this time, if a node that can generate UI is selected in the Elements panel (not all nodes on the DOM tree will eventually generate UI), the Lynx page on the mobile phone will highlight the corresponding UI.
...
3. Now, the corresponding `<image></image>` node is highlighted in the DOM tree.
   <img
...
After selecting a node in the DOM tree, you can use the keyboard to browse the DOM tree.
...
1.  After clicking on the **Inspect** icon in the upper left corner of the developer tools, click on the input box in the middle of the Lynx page preview window.
          ...
          <img src="./assets/elements/elements-key1.png" width="600" style={{ margin: '20px'}}/>

    Now, the `<input>` node is selected in the DOM tree.
...
You can dynamically modify the DOM and see how these changes affect the Lynx page.

### Edit Content

To modify the content of a node, double-click on the corresponding content in the DOM tree.

As shown above:

1. After clicking on the **Inspect** icon, click on the text **Lynx Explorer** in the Lynx page preview window. Now the `<text class="home-title__light">...</text>` node is selected in the DOM tree.
...
3. Press the down arrow key on the keyboard, and the `<raw-text text="Lynx Explorer"><raw-text>` node is selected in the DOM tree.
...
6. Press the Enter key, and you can see that the text change has taken effect both in the DOM tree and on the left preview window.
   <img
...
1. Select the `<view clip-radius="true" flatten="false" style="height:100px">` node in the DOM tree that you want to modify.
...
4. Press the Enter key, and you can see that the width change of the view has taken effect both in the DOM tree and on the left preview window.
   <img
...
2. The `<text class="home-title__light">...</text>` node is selected in the DOM tree.
...
1. Selected `<view clip-radius="true"...>...</text>` in the DOM tree.
...
4. Press the Enter key again. At this time, whether it is the UI effect on the preview window or the DOM tree, the style change has taken effect.
   {' '}
   <img
...
3. Click Go button, then corresponding `<view ...>...</view>` node is selected in the DOM tree.
