...
In daily development, modern front-end frameworks handle most DOM tree and node property updates for us. However, there are times when you need to manipulate elements directly, such as controlling media players, manipulating view behavior, getting element information, or directly modifying styles.
...
However, note that in Lynx, the type of node reference is [`NodesRef`], which is different from React. If you want to learn more about using references, you can refer to [Manipulating the DOM with Refs](https://react.dev/learn/manipulating-the-dom-with-refs).
...
[`lynx.querySelector()`]: api/lynx-api/lynx/lynx-query-selector
