...
Lynx defines content and structure using a markup language, with the most basic unit being an element. The concept of an element is similar to [HTML elements](https://developer.mozilla.org/en-US/docs/Glossary/Element), which can be used to encapsulate different parts of the content to present or operate in a specific manner.

Unlike HTML, Lynx uses some unique elements such as [`<view>`](api/elements/built-in/view), [`<text>`](api/elements/built-in/text), and [`<image>`](api/elements/built-in/image) to display different content. For example, in Lynx, the following source code can be used to display a piece of text:
...
The basic usage of elements is very similar to [HTML elements](https://developer.mozilla.org/en-US/docs/Learn_web_development/Getting_started/Your_first_website/Creating_the_content#anatomy_of_an_html_element):

<img src="/assets/doc/elements_tags.png" width="100%" height="40%" />

Each element consists of the following parts:

1. **Start tag**: Includes the name of the element (in this case, text) surrounded by angle brackets, indicating where the element begins.
2. **End tag**: Similar to the start tag but includes a forward slash before the element's name, indicating where the element ends.
3. **Content**: The content of the element, which for the `<text>` element is the text itself.

Combining the start tag, end tag, and content forms a complete element.

### Attributes

Each element has its own attributes that can be set by adding attribute names and values within the element's tag to describe its behavior and appearance.

For example, each element can use attributes such as [`style`](api/elements/built-in/view#style) and [`class`](api/elements/built-in/view#class) to set background, border-radius, shadow styles, and support some CSS syntax. The following code sets the background color of an element to red:
...
For more attributes, refer to the [Element PAPI reference](api/elements/built-in/view).

### Empty Elements

Some elements do not have content, such as the `<image>` element:

```html
<image src="assets/logo.png" />
```

It does not use an `</image>` end tag, nor does it have any content inside because the `<image>` element uses an attribute `src` to display an image rather than content.

### Nested Elements

Elements can be nested within other elements. For example, multiple `<text>` elements can be nested inside a `<view>` element:
...
This nesting of elements naturally forms a tree structure, which visually describes the hierarchical relationship between elements, known as the [Element Tree](guide/spec.html#element-tree).

The element tree in the source code will be parsed by the Lynx engine at runtime and translated into view trees recognizable by the native rendering systems of different platforms. We rely on this tree structure to build and manage more complex interfaces.

<img src="/assets/doc/elements_tree.png" width="100%" height="30%" />
...
<img src="/assets/doc/elements_hello_lynx_gray.png" width="100%" height="30%" />
...
<img src="/assets/doc/elements_hello_lynx.png" width="100%" height="30%" />
...
<img src="/assets/doc/elements_lynx_logo.png" width="20%" />
...
<img src="/assets/doc/elements_2_na.png" width="100%" height="30%" />
...
<Go
  ...
  img="/assets/doc/elements_lynx_logo_hello_lynx.png"
