...
只有一个无障碍元素才能成功的被读屏软件（iOS 的 VoiceOver 和 Android 的 TalkBack）访问和聚焦。

不过，`<text>` 和 `<image>` 默认就是一个无障碍元素，无需任何操作，他们就可以被识别。

<Go
  example="accessibility"
  img="/assets/doc/a11y.gif"
...
有时，可能希望自己控制无障碍元素的大小，或者是聚合一些无障碍信息，就需要自己控制哪些元素是无障碍节点。

使用 [`accessibility-element`](api/elements/built-in/view#accessibility-element) 标记一个元素是否可以成为一个无障碍元素，允许嵌套。
...
使用 [`accessibility-trait`](api/elements/built-in/view#accessibility-trait) 可以标记一个无障碍元素的特征，可以是图片、按钮、文本中的任意一种。

使用 [`accessibility-label`](api/elements/built-in/view#accessibility-label) 可以调整一个无障碍元素被读屏软件朗读的内容。

:::info
在 `<text>` 上，默认朗读文本的内容
:::

下面的例子中，元素们会被依次阅读成 "Hello lynx" 和 "I am an image displaying the Lynx icon"。
...
iOS 和 Android 系统都会默认以从上到下、从左到右的顺序排列各个无障碍元素，以便它们能够逐个被读屏软件访问。
...
下面的例子中，无障碍元素们的顺序会被调整，会被依次阅读成 "Lynx" 和 "Hello"。
...
使用 [`requestAccessibilityFocus`](api/elements/built-in/view#requestaccessibilityfocus) 提供主动聚焦到某个无障碍元素的能力。
