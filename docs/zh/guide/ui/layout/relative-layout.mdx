...
如果你需要一种布局，在不使用复杂的层次结构的情况下，可以轻松控制子元素与父或兄弟元素的相对位置，那么 Lynx 自研的**相对布局**（relative layout）是最佳选择。

相对布局允许显示子元素在相对位置，每个元素的位置可以相对于兄弟元素（例如，位于另一个元素的左侧或下方）或相对于父区域来指定（例如，对齐底部、左侧或中心）。有关支持的属性，请参见[参考](#参考)部分。
...
要使用相对布局，只需将 `display: relative` 应用于你想要进行相对布局的父元素。
...
通过使用 [`relative-id`](api/css/properties/relative-id)（非 `0` 的整数）为相对布局中的子元素声明 ID，以便更好地识别每个元素。
...
使用以下**边缘对齐样式**，将当前元素与**父或兄弟元素**的某个边缘对齐。例如，使用 [`relative-align-top`](api/css/properties/relative-align-top.mdx) 可指定当前元素与目标元素的上边缘对齐。
...
使用以下**相对位置样式**，指定元素与**兄弟元素**的相对位置。例如，使用 [`relative-left-of`](api/css/properties/relative-left-of.mdx) 指定当前元素的右边缘紧靠着目标兄弟元素的左边缘。
...
使用 [`relative-center`](api/css/properties/relative-center.mdx) 可以指定子元素在容器中的居中方式。通过设置 `vertical` 实现垂直居中，设置 `horizontal` 实现水平居中，或者设置 `both` 同时实现垂直和水平居中。
...
根据渲染效果需求，可以为每个元素赋予相应属性。需要设置元素之间的间隙时，可使用 `margin`。
