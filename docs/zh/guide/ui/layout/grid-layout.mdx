...
如果你想要一个响应式布局，并且包含多个元素同时在垂直和水平方向上交错排列，**网格布局**（grid）是最佳选择。该布局基于二维网格，是 Web 中最强大的 CSS 布局方式。
...
- **容器和网格项（grid container、grid item）**
  ...
  采用网格布局的父元素，称为“容器”（container）。容器内部采用网格定位的子元素，称为“网格项”（grid item）。
...
<center>
  <img width="100%" src="/assets/doc/layout/grid_define.png" />
</center>

### 第一步：应用 `display: grid`

要应用网格布局，只需在父元素上设置 `display: grid`。
...
<Go
  img="/assets/doc/layout/grid_size.png"
  example="layout"
...
<Go
  img="/assets/doc/layout/grid_gap.png"
  example="layout"
...
<Go
  img="/assets/doc/layout/grid_axis_alignment.png"
  example="layout"
...
<Go
  img="/assets/doc/layout/grid_placement.png"
  example="layout"
...
<Go
  img="/assets/doc/layout/grid_area_alignment.png"
  example="layout"
