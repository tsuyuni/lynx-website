...
`background-image` 属性用于为一个元素设置一个或多个背景图像。<br />
在绘制的时候，图像以 z 方向堆叠的方式进行。先指定的图像会在之后指定的图像上面绘制。因此指定的第一个图像 "最接近用户"。<br />
元素的边框 [border](./border.mdx) 会在他们之上被绘制，而 [`background-color`](./background-color.mdx) 会在它们之下绘制。图像的绘制与元素边界和边框 (_border_) 之间的关系，需要在 CSS 属性 [`background-clip`](./background-clip.mdx) 和 [`background-origin`](./background-origin.mdx) 中定义。<br />
